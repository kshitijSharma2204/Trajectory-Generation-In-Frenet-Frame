...
Polynomial::Polynomial(const Eigen::Matrix<double,6,1>& boundaries, double T)
  ...
  Eigen::Matrix<double,6,1> b(6, 1);

  A << 
    1,  0,              0,                 0,                 0,                 0,  // s0
    0,  1,              0,                 0,                 0,                 0,  // v0
    0,  0,              2,                 0,                 0,                 0,  // a0
    1, T_, std::pow(T_,2),    std::pow(T_,3),    std::pow(T_,4),    std::pow(T_,5),  // sT
    0,  1,           2*T_,  3*std::pow(T_,2),  4*std::pow(T_,3),  5*std::pow(T_,4),  // vT
    0,  0,              2,              6*T_, 12*std::pow(T_,2), 20*std::pow(T_,3);  // aT 
  ...
  pos_coeffs_ = A.colPivHouseholderQr().solve(b);
  assert(!pos_coeffs_.hasNaN() && "Polynomial: NaN in position coefficients");
